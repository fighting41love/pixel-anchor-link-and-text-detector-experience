...
历时两个多月，开发的小票OCR店名、金额、时间、单号四个字段全对的一次通过率终于达到了89%。回想起刚接到任务需求的时候，第一反应是这是个不可能完成的任务。一个商场的店家超过100家，每种版式不固定，如何去提取结构化的字段？每个店家去做模板？这种方式扩展性太差，而且同一个商家的小票定时间会升级，维护成本太高。需要一种泛化能力强，又能快速个性化设置的方案。硬着头皮把任务接了下来，方案论证了无数遍，终于理顺了思路。兴冲冲的做了一版初稿，一去测试，一次通过率只有50%，内心顿时跌到了谷底，那种压力感和无助感算法工程师最能体会，有一段时间真的是吃饭都没胃口，所幸一直坚持，趟了无数的坑，终于做出来一个让自己满意的产品。后续我会把遇到的坑一一道来。
